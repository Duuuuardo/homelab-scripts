...
msg_info "Deploying infra stack"
STACK_DIR="/opt/stacks/lxc-infra"
mkdir -p "$STACK_DIR"
cp -r /opt/homelab-scripts/lxc-infra/. "$STACK_DIR/"
cd "$STACK_DIR"
[[ ! -f .env ]] && cp .env.example .env
$STD docker compose pull
$STD docker compose up -d
msg_ok "Deployed infra stack"
...
cp -r /opt/homelab-scripts/lxc-infra/compose.yml .
docker compose pull
docker compose up -d
echo "Infra stack updated."
EOF
